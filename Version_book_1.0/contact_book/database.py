# 
# Мы определили четыре различные функции – create(), read(), update()и delete()
# для каждой из операций, упомянутых выше. 
# Мы используем positionатрибут для идентификации конкретного контакта. 
# change_position() Функция отвечает за сохранение положения контакта всякий раз, когда контакт удаляется.
